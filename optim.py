...
def rmsprop(theta, dtheta, config=None):
  ...
  #############################################################################
  c = config['decay_rate'] * config['cache'] + (1 - config['decay_rate']) * dtheta * dtheta
  next_theta = theta - config['learning_rate'] * dtheta * (1.0/(np.sqrt(c) + config['epsilon']))
  config['cache'] = c
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def adam(theta, dtheta, config=None):
  ...
  v = config['beta2'] * config['v'] + (1 - config['beta2']) * dtheta * dtheta
  t =  config['t'] + 1
